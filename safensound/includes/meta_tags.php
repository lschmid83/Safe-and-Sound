...
if (strpos($_SERVER['PHP_SELF'], 'product_info.php')){
	...
	tep_not_null($product_meta_info['products_model']) && ($show_model_in_desc)
	  ? $desc.=' ('.$product_meta_info['products_model'].')'
	  : NULL;

	tep_not_null($man)
	  ? $desc.=' '.sprintf(META_FROM_MANU, $man)
	  : NULL;
	...
	tep_not_null(HEAD_TITLE_TAG_ALL) && (strlen(STORE_DESCRIPT)>0)
	...
	if(strlen(STORE_DESCRIPT)>0)
		$metadescription.=' '.STORE_DESCRIPT;

	tep_not_null(HEAD_DESC_TAG_ALL) && (strlen(STORE_METDEX)>0)
	...
	if ($current_category_id){
		...
		$desc.=str_replace($strip_bread_array, '',$pre_desc).' '. META_AT_STORE .' '.STORE_NAME;
	...
	# if the page is showing products by manufacturers
		...
		$desc.= sprintf(META_FROM_MANU_AT_STORE, $manufacturers['manufacturers_name'], STORE_NAME);

		// get products by this manufacturer
		$listing_man_sql = tep_db_query("select pd.products_name from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.manufacturers_id = '" . (int)$HTTP_GET_VARS['manufacturers_id'] . "' AND p.products_status = '1' AND pd.products_id = p.products_id AND pd.language_id = '" . (int)$languages_id . "' ORDER BY rand()");
	...
	# index.php default
	if (!isset($HTTP_GET_VARS['manufacturers_id']) && (!$current_category_id))
		$default=true;

	# Final stage for index.php w/o categories
	# Title
	tep_not_null($title)
	  ? $metatitle.=STORE_NAME .' - '.$title
	  : NULL;


	tep_not_null(HEAD_TITLE_TAG_INDEX) && $default==true && (strlen(STORE_METDEX)>0)
	...
	tep_not_null(HEAD_DESC_TAG_ALL) && $default==true && (strlen(STORE_METDEX)>0)
	  ? $desc.=' - '.STORE_METDEX
	  : NULL;

	$metadescription.=$desc;

	tep_not_null(HEAD_DESC_TAG_INDEX) && $default==true && (strlen(STORE_DESCRIPT)>0)
	  ? $metadescription.=' - '.STORE_DESCRIPT
	  : NULL;
