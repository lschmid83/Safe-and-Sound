...
function listTopics($tid="", $findall) {
	$query_topics = tep_db_query("select topic_id, topic_title from " . TABLE_ASNFORUM_TOPIC . " ORDER BY topic_time DESC");
	if (tep_db_num_rows($query_topics)) {
		$combo = '<select name="asnf_search_topic"><option value="-1">' . $findall . '</option>';
		while ($topic_data = tep_db_fetch_array($query_topics)) {
			$combo .= '<option value="'. $topic_data['topic_id'] . '"';
			if ($tid == $topic_data['topic_id']) {
				$combo .= ' selected';
			}
			$combo .= '>' . $topic_data['topic_title'] . '</option>';
		}
		$combo .= '</select>';
	}
	else
	$combo = '<select name="asnf_search_topic"><option value="-1">' . $findall . '</option></select>';

	return $combo;
}

function asnf_error($err_message) {
	    return '
	    <table width="100%" border="0" cellspacing="1" cellpadding="5">
        <tr bgcolor="' . $headercolor . '">
            <td width="100%" class="asnf_normal" align="right"><font color="#ff0000">' .
                 $err_message . ':
            </font></td>
        </tr></table>';
}
