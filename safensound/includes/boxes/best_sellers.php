...
<?php
    ...
    $rows = 0;
    $bestsellers_list = '<table border="0" cellspacing="0" cellpadding="1" style="margin-left:8px">';
    while ($best_sellers = tep_db_fetch_array($best_sellers_query)) {
      $rows++;
      $bestsellers_list .= '<tr><td class="infoBoxContents' . $column_location . '" valign="top">' . tep_row_number_format($rows) . '.</td><td class="infoBoxContents' . $column_location . '"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $best_sellers['products_id']) . '">' . $best_sellers['products_name'] . '</a></td></tr>';
    }
