...
function tep_mark_keywords($tekst, $keywords, $mark_start="<b>", $mark_end="</b>")
  {
   $fldname = $tekst;
   if (keywords != "" )
   {
         $keywords = strtolower($keywords);
         $string_to_split = strtoupper($keywords); // is result UPPERCASE ?
         $tok = split(" ", $string_to_split);
         $size = sizeof($tok);
         for ($i = 0; $i < $size; $i++)
         $fldname = str_replace($tok[$i], $mark_start . $tok[$i] . $mark_end, $fldname);

         $string_to_split = $keywords; // is result LOWERCASE ?
         $tok = split(" ", $string_to_split);
         $size = sizeof($tok);
         for ($i = 0; $i < $size; $i++)
         $fldname = str_replace($tok[$i], $mark_start . $tok[$i] . $mark_end, $fldname);

         $string_to_split = ucwords($keywords); // is result starting with 1 uppercase and rest is lowercase ?
         $tok = split(" ", $string_to_split);
         $size = sizeof($tok);
         for ($i = 0; $i < $size; $i++)
         $fldname = str_replace($tok[$i], $mark_start . $tok[$i] . $mark_end, $fldname);
