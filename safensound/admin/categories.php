        ...
        if (isset($HTTP_POST_VARS['edit_x']) || isset($HTTP_POST_VARS['edit_y'])) {
          ...
          for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
            ...
            $sql_data_array = array('products_name' => tep_db_prepare_input($HTTP_POST_VARS['products_name'][$language_id]),
                                    ...
                                    'products_ebay_url' => tep_db_prepare_input($HTTP_POST_VARS['products_ebay_url'][$language_id]),
                                    'products_amazon_url' => tep_db_prepare_input($HTTP_POST_VARS['products_amazon_url'][$language_id]));
...
<?php
  if ($action == 'new_product') {
    ...
    if (isset($HTTP_GET_VARS['pID']) && empty($HTTP_POST_VARS)) {
        $product_query = tep_db_query("select pd.products_name, pd.products_description, pd.short_desc, pd.products_url, pd.products_ebay_url, pd.products_amazon_url, p.products_id, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, date_format(p.products_date_available, '%Y-%m-%d') as products_date_available, p.products_status, p.products_tax_class_id, p.manufacturers_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "'");
      $product = tep_db_fetch_array($product_query);
      ...
      $products_ebay_url = $HTTP_POST_VARS['products_ebay_url'];
      $products_amazon_url = $HTTP_POST_VARS['products_amazon_url'];
    }
...
?>
          ...
          <!-- eBay product link -->
          <tr>
            <td class="main" style="padding-top:20px">eBay URL:<br></td>
            <td class="main"><?php echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_input_field('products_ebay_url[' . $languages[$i]['id'] . ']', (isset($products_ebay_url[$languages[$i]['id']]) ? stripslashes($products_ebay_url[$languages[$i]['id']]) : tep_get_products_ebay_url($pInfo->products_id, $languages[$i]['id']))); ?></td>
          </tr>
          <!-- Amazon product link -->
          <tr>
            <td class="main" style="padding-top:20px">Amazon URL:<br></td>
            <td class="main"><?php echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_input_field('products_amazon_url[' . $languages[$i]['id'] . ']', (isset($products_amazon_url[$languages[$i]['id']]) ? stripslashes($products_amazon_url[$languages[$i]['id']]) : tep_get_products_amazon_url($pInfo->products_id, $languages[$i]['id']))); ?></td>
          </tr>
<?php
    }
?>
...
<?php
    ...
    if (tep_not_null($HTTP_POST_VARS)) {
      ...
      $products_amazon_url = $HTTP_POST_VARS['products_amazon_url'];
        ...
        $product_query = tep_db_query("select p.products_id, pd.language_id, pd.products_name, pd.products_description, pd.short_desc, pd.products_url, pd.products_ebay_url, pd.products_amazon_url, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.manufacturers_id  from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "'");
      $product = tep_db_fetch_array($product_query);
    ...
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
      if (isset($HTTP_GET_VARS['read']) && ($HTTP_GET_VARS['read'] == 'only')) {
        ...
        $pInfo->products_ebay_url = tep_get_products_ebay_url($pInfo->products_id, $languages[$i]['id']);
        $pInfo->products_amazon_url = tep_get_products_amazon_url($pInfo->products_id, $languages[$i]['id']);
      } else {
        $pInfo->products_name = tep_db_prepare_input($products_name[$languages[$i]['id']]);
        $pInfo->products_description = tep_db_prepare_input($products_description[$languages[$i]['id']]);
        $pInfo->short_desc = tep_db_prepare_input($short_desc[$languages[$i]['id']]);
        $pInfo->products_url = tep_db_prepare_input($products_url[$languages[$i]['id']]);
        $pInfo->products_ebay_url = tep_db_prepare_input($products_ebay_url[$languages[$i]['id']]);
        $pInfo->products_amazon_url = tep_db_prepare_input($products_amazon_url[$languages[$i]['id']]);
      }
?>
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
...
?>
<?php
      if ($pInfo->products_ebay_url) {
?>
      <tr>
        <td><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
      </tr>
      <tr>
        <td class="main"><?php echo sprintf(TEXT_PRODUCT_MORE_INFORMATION, $pInfo->products_ebay_url); ?></td>
      </tr>
<?php
      }
?>
        <?php
      if ($pInfo->products_amazon_url) {
?>
      <tr>
        <td><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
      </tr>
      <tr>
        <td class="main"><?php echo sprintf(TEXT_PRODUCT_MORE_INFORMATION, $pInfo->products_amazon_url); ?></td>
      </tr>
<?php
      }
?>
...
/* Re-Post all POST'ed variables */
      ...
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        ...
        echo tep_draw_hidden_field('products_ebay_url[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($products_ebay_url[$languages[$i]['id']])));
        echo tep_draw_hidden_field('products_amazon_url[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($products_amazon_url[$languages[$i]['id']])));
      }
...
// find out the rating average from customer reviews
      ...
      if (isset($pInfo) && is_object($pInfo) && ($products['products_id'] == $pInfo->products_id) ) {
        echo '              <tr id="Tr1" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $products['products_id'] . '&action=new_product_preview&read=only') . '\'">' . "\n";
      } else {
