...
// Outputs a button in the selected language
  function tep_add_to_cart($image, $alt = '', $parameters = '', $override = false) {
    global $language;
       ...
       $image_submit = '<input type="image" src="images/add_to_basket.png" alt="' . tep_output_string($alt) . '"';
        if (tep_not_null($alt)) $image_submit .= ' title=" ' . tep_output_string($alt) . ' "';
        if (tep_not_null($parameters)) $image_submit .= ' ' . $parameters;
        $image_submit .= '>';
      return $image_submit;
...
// Output a form hidden field
  function tep_draw_hidden_field($name, $value = '', $parameters = '') {
    $field = '<input border="0" type="hidden" name="' . tep_output_string($name) . '"';

    if (tep_not_null($value)) {
      $field .= ' value="' . tep_output_string($value) . '"';
    } elseif (isset($GLOBALS[$name])) {
      $field .= ' value="' . tep_output_string(stripslashes($GLOBALS[$name])) . '" border="0"';
    }
