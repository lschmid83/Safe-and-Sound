...
$fpss_template = "Movies"; // Choose the output template of this slideshow: "Default", "FSD", "Movies", "JJ-Obs", "JJ-Rasper", "Movies", "Sleek", "TT", "Uncut". See them all live on: www.frontpageslideshow.net
...
$width = 500; // Slideshow Width in pixels. Please note that this value references to the slide viewport. If you are using a template like Movies, whoich utilizes a sidebar, the total width of this slideshow will be the sum of this value and the sidebar width value (set below). Images exceeding this Width will display cropped without messing your slideshow.
$height = 308; // Slideshow Height in pixels. Please note that this value references to the slide viewport. Images exceeding this Height will display cropped without messing your slideshow.
$sidebar_width = 200; // Width of the Sidebar in pixels. Concerns the 'Uncut', 'FSD', 'Movies' and 'Sleek' templates by default. This width will be added to the slideshow Width set above.
...
$striptags = 1; // Cleanup HTML tags from slide text? 0 for "no", 1 for "yes". Default is 1. If you choose '1' then all HTML tags will be stripped from the main text of your slide, except the tags specified in the mod_fpslideshow.php file. It's best if you leave this to 1.
$openlinksexternally = 0;
$random_ordering = 0; // Display slides randomly? 0 for "no", 1 for "yes". Default is 0. This option will shuffle the slides in your slideshow.
